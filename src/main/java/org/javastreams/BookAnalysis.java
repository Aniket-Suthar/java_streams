...
public class BookAnalysis {
    public static String analyzeBooks(List<Book> books) {
        ...
        Map<String, Set<String>> popularAuthors = books.stream()
                ...
                .collect(Collectors.groupingBy(b -> b.author, Collectors.mapping(b -> b.category, Collectors.toSet())));
        ...
        books.stream().collect(Collectors.groupingBy(b -> b.category,
        ...
        books.stream().collect(Collectors.groupingBy(b -> b.edition))
                .forEach((edition, list) -> {
                    int totalDiscounted = list.stream().filter(b -> b.discount).mapToInt(b -> b.copiesLeft).sum();
                    double avgNonDiscounted = list.stream().filter(b -> !b.discount).mapToInt(b -> b.copiesLeft).average().orElse(0);
        ...
        int top10Threshold = books.stream().mapToInt(b -> b.wishedUsers).sorted().skip((long) (books.size() * 0.9)).findFirst().orElse(0);
        books.stream().collect(Collectors.groupingBy(b -> b.author, Collectors.summingInt(b -> b.wishedUsers)))
                .entrySet().stream().filter(e -> e.getValue() >= top10Threshold)
        ...
        books.stream().collect(Collectors.groupingBy(priceCategory, Collectors.groupingBy(lengthCategory)))
                .forEach((price, map) -> {
